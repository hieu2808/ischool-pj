...
<form action="{{ route('postCreateClass', ['mon_hoc_id' => request()->mon_hoc_id]) }}" method="post">
    ...
    <div class="mb-3">
        ...
        <input type="text" class="form-control" name='ten_lop_hoc' id="tenLopHoc" required>
    </div>
    <div class="mb-3">
        <label for="soLuongSV" class="form-label">Số lượng sinh viên</label>
        <input min="1" max="100" type="number" name='so_luong_sv' class="form-control" id="soLuongSV" required>
    </div>
    <div class="mb-3">
        <label for="soTinChi" class="form-label">Số tín chỉ</label>
        <input min="1" max="5" type="number" name='so_tin_chi' class="form-control" id="soTinChi" required>
    </div>
    <div class="mb-3">
        <label for="ngayDangKy" class="form-label">Ngày bắt đầu</label>
        <input type="date" name='ngay_bat_dau' class="form-control" id="ngayDangKy" required>
    </div>
    <div class="mb-3">
        <label for="ngayKetThuc" class="form-label">Ngày kết thúc</label>
        <input type="date" name='ngay_ket_thuc' class="form-control" id="ngayKetThuc" required>
    </div>
